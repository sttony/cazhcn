...
\begin{exmp}\label{Exmp:CreateAndAccessAClass}
    创建和存取一个Java 对象
...
\begin{lstlisting}[language={Java}, keywordstyle=\color{blue!70}, commentstyle=\color{red!50!green!50!blue!50}]
    while("continuation condition")
        body;
    for("initializer"; "continuation condition"; "incrementer")
        body;
\end{lstlisting}
...
仅拷贝引用，而不是对象的实例域，就像我们在例\ref{Exmp:CreateAndAccessAClass}中看到的。如果这些变量在一个叫
...
\begin{exmp}
    ...
    在图\ref{Fig:OrgDataClasswithOrgYear}中，为例\ref{Exmp:CreateAndAccessAClass}的
...
\begin{defin} \label{Def:RelationAndAttribute}
    关系的重要属性
...
\begin{defin} \label{Def:Logarithms}
    对数函数和对数底
...
\begin{defin} \label{Def:ConditionalProbability}
条件概率
...
\begin{exmp}\label{Exmp:CondtionalProbabilityOfTwoDice}
两个骰子的条件概率
...
事件的条件概率可能比事件的非条件概率要大或是小。在例\ref{Exmp:CondtionalProbabilityOfTwoDice}
中$S_1$的非条件概率是1/6，在给出$S_3$的情况下条件概率是1/2。另一方面，“A的数字能被3整除”
的非条件概率是1/3。但是在例\ref{Exmp:CondtionalProbabilityOfTwoDice}中，我们看到给出$S_3$
...
继续我们在例\ref{Exmp:CondtionalProbabilityOfTwoDice}中的事件定义，事件
$S_1$和$S_2$是相互独立的，因为每一个的概率都是1/6，而且（$S_1$ and
$S_2$）组成一个基本事件，它的概率是1/36。注意，$Pr(S_1|S_2)=(1/36)(6/36)=1/6=Pr(S_1)$
。

从例\ref{Exmp:CondtionalProbabilityOfTwoDice}的讨论，我们可以看出$S_1$
...
\begin{exmp}\label{Exmp:ConditionalProbabilityAndSequence}
条件概率和顺序
...
考虑象例\ref{Exmp:ConditionalProbabilityAndSequence}那样的概率空间。让我们定义随机变量
...
\begin{lemma}\label{Lemma:ConvexLemma1}
\begin{enumerate}
...
\begin{lemma}\label{Lemma:ConvexLemma2}
\begin{enumerate}
...
\begin{exmp}\label{Exmp:SearchInUnSortedArray}
在无序数组中查找
...
\begin{defin}\label{Def:GeneralSearch}
普通查找例程
...
机会$K$不在数组中，则$A(n)=3n/4+1/4$；大概要检查3/4的数组元素。这是例\ref{Exmp:SearchInUnSortedArray}
的结论。
\end{algorithm}

例\ref{Exmp:SearchInUnSortedArray}展示了我们如何解释规模为n的输入集合$D_n$。
...
最后，例\ref{Exmp:SearchInUnSortedArray}展示了在进行查找和排序的平均分析时经常
用到的假设；所有的元素都是不同的。对不同元素的平均分析给出了对重复元素较少情况的
比较接近的平均行为。如果这里有很多重复元素，就很难对K第一次出现在数组的什么地方做
出合理的假设。

\begin{exmp}\label{Exmp:MatrixMultiplication}
矩阵乘法
...
对于$m=n=p$时，$A(n)=W(n)=n^3$。这是例\ref{Exmp:MatrixMultiplication}的结论。


\end{algorithm}

例\ref{Exmp:MatrixMultiplication}展示了有些算法只是执行指令，因此工作量
...
在最坏情况下算法低限的概念对于计算复杂性是十分重要的。例\ref{Exmp:FindMaxValueInArrary}
...
\begin{exmp}\label{Exmp:FindMaxValueInArrary}
查找数组中最大的元素
...
例\ref{Exmp:FindMaxValueInArrary}的结论。
\end{algorithm}

在建立例\ref{Exmp:FindMaxValueInArrary}的低限中我们可能需要一些稍微不同的
...
上限：常用的算法（参见例\ref{Exmp:MatrixMultiplication}）做$n^3$
次乘法，因此至多需要$n^3$。
...
有一天Anchorage会比El Paso暖和吗？这是肯定的；当寒流传到El Paso时，Anchorage很
...
\subsection{渐近阶的定义和符号}\label{Sec:AsymptoticOrderDefineAndSymbols}
我们将使用一些自然数和实数的常用记号。
\begin{defin}
自然数和实数的符号
\begin{enumerate}
...
这里有两个有用的定例。他们的证明用到了在\ref{Sec:AsymptoticOrderDefineAndSymbols}
...
我们假定，就像我们在例\ref{Exmp:SearchInUnSortedArray}中做的，K在数组
...
\end{displaymath}
\noindent
第一个等式展示了K在数组中的情况，与例\ref{Exmp:SearchInUnSortedArray}中的
相同。第二个等式展示了K不在数组中的情况。估计和很简单，留到了练习中。
就像在例\ref{Exmp:SearchInUnSortedArray}中一样，最后的结果通过等式
...
\begin{figure*}[!t]
    ...
    \begin{tikzpicture}
        ...
        \filldraw[fill=red!40!green!40!blue!40] (4,3) circle(8pt) node{4};
        \filldraw[fill=red!40!green!40!blue!40] (2, 2) circle (8pt) node{1};
        \filldraw[fill=red!40!green!40!blue!40] (6, 2) circle (8pt) node{7};
        \filldraw[fill=red!40!green!40!blue!40] (1,1) circle (8pt) node{0};
        \filldraw[fill=red!40!green!40!blue!40] (3,1) circle (8pt) node{2};
        \filldraw[fill=red!40!green!40!blue!40] (5,1) circle (8pt) node{5};
        \filldraw[fill=red!40!green!40!blue!40] (7,1) circle (8pt) node{8};
        \filldraw[fill=red!40!green!40!blue!40] (3.5,0) circle (8pt) node{3};
        \filldraw[fill=red!40!green!40!blue!40] (5.5,0) circle (8pt) node{6};
        \filldraw[fill=red!40!green!40!blue!40] (8,0) circle (8pt) node{9};
    \end{tikzpicture}
